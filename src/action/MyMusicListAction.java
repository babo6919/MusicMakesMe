	...
	//Action 인터페이스의 execute 실체 메소드 작성
		public ActionForward execute(HttpServletRequest request, HttpServletResponse response)throws Exception{
			...
			if(ID.equals(null)) {
				...
				forward = new ActionForward();
				MyMusicListService mymusiclistservice=new MyMusicListService();
				//컬렉션 프레임 워크 중의 하나인 ArrayList 사용(자바 15장 내용참고)
				//MemberBean타입의 ML를 사용하여 조회된 전체 회원의 정보를 가져옴.
				ArrayList<Possesion_MusicBean> mymusicList = mymusiclistservice.getMusicList(ID);
				System.out.println("액션 : " + mymusicList);
				request.setAttribute("mymusicList", mymusicList);
				forward.setPath("./Main.jsp?page=MyMusic");
			}
			//2.admin인지 확인
